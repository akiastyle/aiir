...
curl -fsS "http://${HOST}:${PORT}/openapi.json" >"$TMPDIR/openapi.json"
...
if ! rg -q '"openapi":"3\.0\.3"' "$TMPDIR/openapi.json"; then
  echo "smoke-failed: missing openapi payload" >&2
  cat "$TMPDIR/openapi.json" >&2 || true
  exit 1
fi
